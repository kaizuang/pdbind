...
\paragraph{Workload Split}
We plan to divide the project responsibilities as follows. Both authors will jointly design and implement the dual GAT encoder, the cross-attention fusion module, MLP regression head, and the training loop. From there, each author will independently take ownership of a subset of the protein graph constructions---one focusing on the $C_\alpha$-based adjacency configurations and the other on the MHAD-based configurations---running experiments and analyzing performance across their respective graph representations. Both authors will then collaborate to compare results across all configurations and jointly write the final report.
